...
The same completion works at the shell prompt — `eval "$(juxt-complete --bash)"` in `~/.bashrc` (see [Installation](https://juxt.readthedocs.io/en/latest/installation/)) — so `juxt plots/{sensor}/2⇥` completes there too.
